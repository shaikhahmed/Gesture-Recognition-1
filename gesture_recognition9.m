...
for image_index=68:80
    ...
    x_line1=zeros(1,row);
    x_line1(1:y_center)=x_center+round((y_center-(1:y_center))./tand(direction_angle1));
    x_line1(y_center+1:row)=x_center-round(((y_center+1:row)-y_center)./tand(direction_angle1));
    ...
    for i=1:length
        ...
        k=1;
        while(true)
            [fingeredge_start,fingeredge_end]=finger_finder3(x_array,y_array,fingertip(i),[fingeredge_start,fingeredge_end],direction,start_index,i);
            if(firstTime)
                firstTime=~firstTime;
                ...
                Q=[P(1)+10,P(2)-10/tand(direction(i)/2)];
            ...
            area_percent(k+1)=finger_area/area;
            fingerroots(k,:)=[fingeredge_start,fingeredge_end];
            if((area_percent(k)>0.5&&area_percent(k+1)<area_percent(k))||(fingeredge_start>start_index+fingertip(i)-1||fingeredge_end<start_index+fingertip(i)-1))
                fingeredge([2*i-1,2*i])=fingerroots(k-1,:);
                break;
            end
            fingeredge_start=fingeredge_start+1;
            fingeredge_end=fingeredge_end-1;
            k=k+1;
        end
    end
...
%     end
    ...
    imagesc(edge_map);
    hold on;
    plot(x_line,y_line,'r',x_line1,y_line,'w',x_center,y_center,'w*',x_start,y_start,'r*',x_end,y_end,'r*');
    plot(x_array(start_index+fingervalley-1),y_array(start_index+fingervalley-1),'w*');
    ...
    for i=1:find(fingertip,1,'last')
        x_line1(1:y_array(start_index+fingertip(i)-1))=x_array(start_index+fingertip(i)-1)+round((y_array(start_index+fingertip(i)-1)-(1:y_array(start_index+fingertip(i)-1)))./tand(direction(i)/2));
        x_line1(y_array(start_index+fingertip(i)-1)+1:row)=x_array(start_index+fingertip(i)-1)-round(((y_array(start_index+fingertip(i)-1)+1:row)-y_array(start_index+fingertip(i)-1))./tand(direction(i)/2));
        plot(x_line1,y_line,'k');
    end
